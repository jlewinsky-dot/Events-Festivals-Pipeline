...
SYSTEM_PROMPT = (
    ...
    "farmers markets (these are outdoor community markets that need portable restrooms), "
    "flea markets, swap meets, outdoor trade days, "
    "- Golf tournaments, golf championships, PGA events, golf qualifiers"
    ...
    "- Night markets, indoor craft markets at existing permanent venues\n"
    ...
    "- If event name contains park, say YES.\n"
...
def build_event_list(events):
    ...
    for i, event in enumerate(events):
        ...
        location = event.get("search_city", "")
...
def filter_relevant_events(events, batch_size=200):
    if not events:
        return []
    ...
    logger.info(f"Relevance filter done: {len(relevant)} relevant / {len(events)} total")
    return relevant