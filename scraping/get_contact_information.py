...
def extract_event_info(event_title:str, home_html:str, contact_html=None, about_html=None, organizer_url=None) -> list:
    ...
    if attendees_source is None and estimated_attendees is not None:
        attendees_source = 'estimated'

    if isinstance(estimated_attendees, str):
        cleaned = estimated_attendees.replace(',', '').strip()
        if cleaned.isdigit():
            estimated_attendees = int(cleaned)
        else:
            try:
                estimated_attendees = int(float(cleaned))
            except Exception:
...
def search_missing_fields(event_title, location, contact_info):
    ...
    try:
        ...
        if contact_info[6] is None:
            ...
            if contact_info[6] is not None and contact_info[7] is None:
                contact_info[7] = 'estimated'
