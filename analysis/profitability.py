...
SYSTEM_PROMPT = (
    ...
    "For each event, consider: estimated attendance, whether it's outdoor, duration, overall popularity, "
    "and these fields: sells_food, sells_alcohol, sells_vip, estimated_attendees. "
...
# Creating a big numbered string for each event to pass in batches to LLM
def _build_event_list(events):
    lines = []
    for i, event in enumerate(events):
        title = event.get("title", "")
        address = event.get("address", "")
        sells_food = event.get("sells_food", "")
        sells_alcohol = event.get("sells_alcohol", "")
        sells_vip = event.get("sells_vip", "")
        estimated_attendees = event.get("estimated_attendees", "")
        lines.append(f"{i+1}. {title} | {address} | sells_food: {sells_food} | sells_alcohol: {sells_alcohol} | sells_vip: {sells_vip} | estimated_attendees: {estimated_attendees}")
    return "\n".join(lines)
